...
export async function getAvailableRooms(checkInDate, checkOutDate, roomType){
    const result = await api.get(`/rooms/availableRooms?checkInDate=${checkInDate}&checkOutDate=${checkOutDate}&roomType=${roomType}`);
    return result;
}
...
export async function deleteUser(userId){
    try{
        const response = await api.delete(`/users/${userId}`, {headers : getHeader()});
        return response.data;
    }catch(error){
...
export async function getUser(userId, token){
    try{
        const respone = await api.get(`/users/${userId}`, {headers: getHeader()})
        return respone.data;
    }catch(error){
        return error.message;
    }
